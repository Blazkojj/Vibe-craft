...
function Legal() {
  ...
  let title = isEN ? "Legal Document" : "Dokument Prawny";
  let content = isEN ? "Loading document..." : "Trwa ładowanie dokumentu...";

  if (path === '/regulamin') {
    title = isEN ? "Shop Regulations / ToS" : "Regulamin Sklepu";
    content = isEN ? `
      1. General Provisions
      These regulations define the rules for providing electronic services within the Zenexcode platform (hereinafter referred to as the "Service"). 
      Using the Service implies acceptance of these regulations. 
      Zenexcode reserves the right to modify these regulations, and users will be notified of any changes.

      2. Services and Payments
      The Zenexcode Service enables automatic generation of code (Minecraft plugins) using artificial intelligence.
      Access to premium features requires topping up a virtual wallet (credits in USD) or purchasing a plan.
      Payments are handled exclusively by a certified operator - the Suppi.pl platform.
      All prices listed in the Service are gross prices and include applicable taxes.

      3. Liability
      The user acknowledges that code generated by AI may contain errors or security vulnerabilities. 
      Zenexcode is not liable for any losses, server damage, or data loss resulting from the use of the generated plugins.

      4. Intellectual Property
      The generated code becomes the property of the user. However, we reserve the right to anonymize and use queries (prompts) to improve our models.

      5. Refunds and Complaints
      Due to the digital nature of the services and their immediate execution (API generation), 
      the user generally has no right to refunds for spent credits. 
      Complaints regarding errors in the operation of the panel itself should be directed to our Discord server.
    ` : `
    ...
    title = isEN ? "Privacy Policy" : "Polityka Prywatności";
    content = isEN ? `
      1. Who processes your data?
      The administrator of your personal data is the Zenexcode team. 
      We make sure that your data is safe and processed in accordance with GDPR (RODO).

      2. What data do we collect?
      - Email address (for login and account verification using the Supabase system).
      - Your nickname from the Suppi.pl platform (in order to assign purchased packages to your account).
      - History of queries (prompts) to our AI to provide the service.

      3. Data Sharing
      Your data is not sold to third parties. Part of the queries may be processed by external AI services (e.g. Google Gemini, Anthropic) as part of the service.
      We use external operators for authentication (Supabase) and payments (Suppi.pl).

      4. Cookies
      The Service uses only necessary cookies to maintain user login sessions. 
      We do not use tracking cookies for advertising purposes.

      5. Your rights
      You have the right to view your data, correct it, and request complete deletion of your account ("right to be forgotten"). 
      For this purpose, please contact us on our Discord server.
    ` : `
    ...
    title = isEN ? "Terms of Use" : "Warunki Korzystania";
    content = isEN ? `
      1. AI Usage Rules
      The user agrees not to generate illegal content, promote violence, or create malicious software (e.g. malware, backdoors, force-op plugins). 
      We reserve the right to immediately suspend accounts in case of such attempts.

      2. Fair Use Policy
      "Unlimited" plans are subject to Fair Use. 
      This means that unlimited access applies to normal use by a single human. 
      Using bots, automated query scripts, or sharing accounts is prohibited.
      In case of abnormally high resource usage, Zenexcode may temporarily rate limit generation for that account.

      3. Service Availability (SLA)
      We strive to keep Zenexcode available 24/7. 
      However, due to dependence on external LLM providers, temporary downtime may occur.
    ` : `
    ...
    <div className="dev-landing" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div className="dev-container" style={{ textAlign: 'left', alignItems: 'flex-start', flex: 1, padding: '4rem 2rem' }}>
        ...
        <Link to="/" style={{ color: 'var(--text-muted)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '3rem' }}>
          <ArrowLeft size={18} /> {isEN ? "Go back to homepage" : "Wróć na stronę główną"}
        </Link>
