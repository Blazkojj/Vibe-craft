    ...
    <div className="claude-sidebar" style={{ width: isCollapsed ? '64px' : '260px' }}>
      ...
      {!isCollapsed && isSearchOpen && (
        <div className="sidebar-search-box">
          <input 
            type="text" 
            placeholder={isEN ? "Search project..." : "Szukaj projektu..."} 
            value={searchQuery}
      ...
      <div className="sidebar-new-chat">
        <Link to="/dashboard" className="new-chat-btn">
          <span style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: isCollapsed ? 'center' : 'flex-start' }}>
            <Plus size={16} /> {!isCollapsed && (isEN ? "New project" : "Nowy projekt")}
          </span>
        </Link>
      </div>
      ...
      <div className="sidebar-nav">
        ...
        <Link to="/ustawienia" className={`sidebar-nav-item ${location.pathname === '/ustawienia' ? 'active' : ''}`} title={isEN ? "Settings & Wallet" : "Ustawienia & Portfel"}>
          <SettingsIcon size={16} /> {!isCollapsed && (isEN ? "Settings & Wallet" : "Ustawienia & Portfel")}
        </Link>
      </div>

      <div className="sidebar-recents" style={{ display: isCollapsed ? 'none' : 'block' }}>
        <div className="recents-title">{isEN ? "Recent" : "Ostatnie"}</div>
      ...
      <div className="sidebar-footer">
        <button className="user-profile-btn" onClick={handleLogout} style={{ justifyContent: isCollapsed ? 'center' : 'flex-start' }} title={isEN ? "Log out" : "Wyloguj"}>
          ...
          {!isCollapsed && <span className="user-name">{user?.user_metadata?.discord_profile?.global_name || user?.user_metadata?.discord_profile?.username || user?.user_metadata?.username || (isEN ? 'Log out (Account)' : 'Wyloguj (Konto)')}</span>}
        </button>
      </div>
    </div>
  );
