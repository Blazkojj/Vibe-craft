...
conn.on('ready', () => {
  console.log('[SSH] Testowe uruchomienie serwera Minecraft...\n');
...
echo "Sending power action START...\n";
try {
    $res = $repo->power('start');
    echo "Power start response code: " . $res->status() . "\n";
} catch (\\Exception \$e) {
    echo "Power start Exception: " . \$e->getMessage() . "\n";
}
`;

  conn.exec(`cat << 'PHPEOF' > /tmp/test_power.php\n${phpScript}\nPHPEOF\nphp /tmp/test_power.php`, (err, stream) => {
    if (err) throw err;
